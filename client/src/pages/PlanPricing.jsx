...
const Pricing = () => {
  ...
  const handleUpgrade = async (planId) => {
    if (planId === 'free' || planId === user?.current_plan) return;
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 md:py-12 px-3 md:px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-sm md:max-w-none mx-auto">
        {plans.map((plan) => {
          const isCurrentPlan = plan.id === (user?.current_plan || 'free');
              ...
              >
                {loading === plan.id ? (
                  ...
                  <>
                    ...
                    {!isCurrentPlan && <ArrowRight size={16} className="md:w-[18px] md:h-[18px]" />}
                  </>
                )}
              </button>
            </div>
